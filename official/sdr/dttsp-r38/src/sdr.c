...
setup_tx(void) {
  ...
  tx->osc.gen = newOSC(uni->buflen,
		       ComplexTone,
		       tx->osc.freq,
		       tx->osc.phase, uni->samplerate,
		       "SDR TX Oscillator");


  tx->am.carrier_level = 0.5;
  tx->fm.cvtmod2freq = 2500.0 * TWOPI / uni->samplerate;//5 kHz deviation
  ...
  tx->leveler.gen = newDttSPAgc(1,	// mode kept around for control reasons
				...
				500,	//Hangtime in ms
				uni->samplerate,	// Sample rate
				5.62,	// Maximum gain as a multipler, linear not dB
  ...
  tx->alc.gen = newDttSPAgc(1,	// mode kept around for control reasons alone
			    ...
			    0.000001,	// Minimum gain as a multipler, linear not dB
			    1.0,	// Set the current gain
			    "ALC");	// Set a tag for an error message if the memory allocation fails
  tx->alc.flag = TRUE;


  tx->spr.gen = newSpeechProc(0.4, 3.0, CXBbase(tx->buf.i), CXBsize(tx->buf.o));
  tx->spr.flag = FALSE;
...
do_rx_meter(int k, CXB buf, int tap) {
  ...
  case RXMETER_PRE_CONV:
    for (i = 0; i < len; i++)
      uni->meter.rx.val[k][ADC_REAL] = max(fabs(vec[i].re), uni->meter.rx.val[k][ADC_REAL]);
    uni->meter.rx.val[k][ADC_REAL] = dBP(uni->meter.rx.val[k][ADC_REAL]);
    for (i = 0; i < len; i++)
      uni->meter.rx.val[k][ADC_IMAG] = max(fabs(vec[i].im), uni->meter.rx.val[k][ADC_IMAG]);
    uni->meter.rx.val[k][ADC_IMAG] = dBP(uni->meter.rx.val[k][ADC_IMAG]);
    break;
  ...
  case RXMETER_POST_FILT:
    ...
    uni->meter.rx.val[k][SIGNAL_STRENGTH] = Log10P(uni->meter.rx.val[k][SIGNAL_STRENGTH]);
...
do_rx_post(int k) {
  ...
  if (rx[k]->squelch.set) do_rx_squelch(k);
  else {
    no_rx_squelch(k);
    // spotting tone
    if (rx[k]->spot.flag) {
  ...
  if (!rx[k]->bin.flag)
    for (i = 0; i < n; i++)
      CXBdata(rx[k]->buf.o, i) = Cscl(rx[k]->azim, M_SQRT2 * CXBreal(rx[k]->buf.o, i));
}
...
do_tx_meter(CXB buf, TXMETERTYPE mt) {
  ...
  case TX_ALC:
    for (i = 0; i < CXBhave(tx->buf.i); i++)
      tx->sav.alc = DamPlus(tx->sav.alc, Csqrmag(CXBdata(tx->buf.i, i)));
    uni->meter.tx.val[TX_ALC]   = Log10Q(tx->sav.alc);
    uni->meter.tx.val[TX_ALC_G] = dBP(tx->alc.gen->gain.now);;
    break;
...
do_tx_AM(void) {
  ...
  for (i = 0; i < n; i++)
    CXBdata(tx->buf.i, i) =
      Cmplx(tx->am.carrier_level + (1.0 - tx->am.carrier_level) * CXBreal(tx->buf.i, i),
	    0.0);
}
