...
newPolyPhaseFIR(int filterMemoryBuffLength,
		...
		int filterPhaseNum,
		int deciFactor) {
  ResSt tmp = (ResSt) safealloc(1, sizeof(resampler), "PF Resampler");
  ...
  tmp->filterMemoryBuff = (COMPLEX *) safealloc(tmp->filterMemoryBuffLength,
						sizeof(COMPLEX),
						"Filter buff: resampler");
  tmp->filter = newFIR_Lowpass_REAL(0.45f / (REAL) deciFactor,
				    (REAL) interpFactor,
				    31 * interpFactor);
  return tmp;
}
