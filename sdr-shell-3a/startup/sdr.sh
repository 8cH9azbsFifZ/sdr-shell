...
JACKDRT=/usr/local/bin/jackd
JACKC=/usr/local/bin/jack_connect
DTTSP=/usr/local/bin
...
if [ "$SND_DRIV" == "oss" ]; then
  ...
  if [ "$SND_DRIV" == "alsa" ]; then
    ...
    # Start dttsp
    echo "> Starting dttsp..."
    $DTTSP/sdr-core -s -m --client-name=Dttsp&
    DTTSP_PID=$!
    JACKD_PID=$!
    if [ $DTTSP_PID ] 
    then
...
if [ "$SND_DRIV" = "oss" ]; then
    ...
    sleep 3 
    echo "  Dttsp:ol -> oss:playback_1"
    $JACKC Dttsp:ol oss:playback_1 
    echo "  Dttsp:or -> oss:playback_2"
    $JACKC Dttsp:or oss:playback_2 
    echo "  oss:capture_1 -> Dttsp:il"
    $JACKC oss:capture_1 Dttsp:il 
    echo "  oss:capture_2 -> Dttsp:ir"
    $JACKC oss:capture_2 Dttsp:ir 
else 
if [ "$SND_DRIV" = "alsa" ]; then
 echo "> Connecting dttsp to jack..."
    sleep 3 
echo "  Dttsp:ol -> alsa_pcm:playback_1"
    $JACKC Dttsp:ol alsa_pcm:playback_1
    echo " Dttsp:or -> alsa_pcm:playback_2"
    $JACKC Dttsp:or alsa_pcm:playback_2
    echo "  alsa_pcm:capture_1 -> Dttsp:il"
    $JACKC alsa_pcm:capture_1 Dttsp:il
    echo "  alsa_pcm:capture_2 -> Dttsp:ir"
    $JACKC alsa_pcm:capture_2 Dttsp:ir
fi
fi
